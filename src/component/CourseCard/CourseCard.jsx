...
const CourseCard = ({image,topic, explain, teacher, time, price, id}) => {
  return (
   
    <div className=" relative w-[31%] max-sm:w-[95%] max-md:w-[31%] md:max-lg:w-[30%] h-[325px] max-md:h-[300px] md:max-lg:h-[220px] bg-white rounded-[20px] dark:dark:bg-[#29435c] dark:text-[#d1d4c9]">
      ...
      <ul className="relative pt-3 mr-2 max-md:text-[11px] max-md:mt-[1px] max-md:pt-[0] md:max-lg:top-[-10px] text-[13px] md:max-lg:text-[10px] list-disc list-inside text-[#0004ff] dark:text-[#d1d4c9]"><li>{teacher}</li></ul>
      <div  className="border-t border-slate-400 w-[90%] mx-auto pt-2 max-lg:pt-1">
        <Link to={'/coursedetail/'+id} className="text-[13px] max-lg:text-[10px] px-1 py-2 rounded-xl bg-slate-400 dark:dark:bg-[#556e53] dark:text-[#d1d4c9]">جزئیات دوره</Link>
      </div>
      ...
      <span className="absolute mt-6 md:max-lg:mt-2  max-sm:right-[3%] max-md:right-[4%]  max-md:text-[12px]  right-[10px] max-lg:right-[8px] max-lg:text-[11px] text-[15px] max-xl:text-[14px] text-red-700 dark:text-[#d1d4c9]" >{price} تومان </span>
      </div> 
  )
}
