

function Services() {
  return (
    <section className="py-12 overflow-x-hidden bg-[url(public/line.png)] [background-size:100%_100%] max-sm:[background-size:100%_30%] max-sm:[background-position:50%]  bg-no-repeat">
    <div className="container mx-auto px-4 ">
      <h2 className="text-2xl font-bold text-center mb-8">خدمات</h2>
      <div className=" p-6 rounded-lg shadow-md text-center">
        <h3 className="font-bold text-lg mb-4">مدرک معتبر</h3>
        <p className="text-gray-600">
          مدرک معتبر را با ما دریافت کنید که قابل ارائه به موسسات رسمی است و
          معتبر برای کسب و کارها.
        </p>
      </div>
    </div>
  </section>
  )
}

export default Services