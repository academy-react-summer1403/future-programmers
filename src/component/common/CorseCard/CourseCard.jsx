...
const CourseCard = ({ title, image, describe, teacher, id , userIsLiked, userIsDissLiked, userLikedId, dissLikeCount, likeCount, levelName, courseRate, isUserFavorite, userFavoriteId, cost, setReFetch}) => {
    ...
    return(
        ...
        <h1 className="text-[14px] w-[90%] max-md:text-[12px] max-lg:text-[9px] font-bold pr-4 relative bottom-5 dark:text-[#d1d4c9]">{title}</h1>
        ...
        <div className="flex gap-3">
            ...
            <img onClick={()=>handleFavorit(id)} src={isUserFavorite === true ? saved : save} className='w-5 max-lg:w-4 h-5 max-lg:h-4 cursor-pointer' alt="" ></img>
        </div>
