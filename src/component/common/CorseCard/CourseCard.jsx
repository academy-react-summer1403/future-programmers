...
const CourseCard = ({ title, image, describe, teacher, id , userIsLiked}) => {
    const handleLike = async () => {
        const res = await http.post(`/Course/AddCourseLike?CourseId=${id}`)
        console.log(res)
    }
    return(
        <div className=" relative w-[23%] max-sm:w-[95%] max-md:w-[45%] max-md:h-[270px] h-[290px] md:max-lg:h-[220px] bg-white rounded-[20px] dark:dark:bg-[#29435c] dark:text-[#d1d4c9]">
            ...
            <ul className="relative pt-3 mr-2 max-md:text-[11px] max-md:mt-[1px] max-sm:pr-3 max-md:pt-[0] md:max-lg:top-[-10px] text-[13px] md:max-lg:text-[10px] list-disc list-inside text-[#0004ff] dark:text-[#d1d4c9]"><li>{teacher}</li></ul>
            <div  className="border-t border-slate-400 w-[90%] mx-auto mt-2 pt-3 max-lg:pt-1">
                <Link to={'/coursedetail/'+id} className="text-[13px] max-lg:text-[10px] max-md:mt-2 max-lg:mt-3 px-1 py-2 rounded-xl bg-slate-400 dark:dark:bg-[#556e53] dark:text-[#d1d4c9]">جزئیات دوره</Link>
