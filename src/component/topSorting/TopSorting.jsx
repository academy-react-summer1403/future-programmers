...
function TopSorting({setSort}) {
  ...
  return (
    <div className='items-center max-md:hidden md:flex mr-[25px] max-xl:mr-[5px] max-lg:mr-[15px]'>
        <ul className='flex item-center' onChange={(e)=>setSort(e.target.value)}>
          {/* <li className="font-bold pl-[10px] text-[16px] max-lg:pl-[0px] max-lg:content-center max-lg:text-[11px] dark:text-[#d1d4c9]">مرتب سازی</li>
        ...
        {data.map((item, index)=>{
          return(<li key={index} value={item.value}>
            <input type="radio" value={item.value} id={item.lable}  name='sort' className="" />
